<?php
function update_withdrawal_history($username, $date, $time, $account_number, $amount, $service_charge, $bank, $reciepient_code, $account_name, $reference, $status, $reason,$id){
    ...
    <p class='lead'>Transfer of $amount to ".$_POST["sender"]." | ".$_POST["bank"]." $status</p>
...
if(isset($_POST["amount"])){
    if($_POST["amount"] != "" || $_POST["amount"] >0){
        ...
        $_SESSION["rrr"] = "You have Successfully Debited this user's wallet";
        $date_t = date("d/m/y");
        $time_t = date("h:i A");
        $reference = (rand(1111111111,9876543210) +1);
        update_withdrawal_history($username, $date_t, $time_t, "".$_POST["acc_no"], $am, "0", $_POST["bank"], $_POST["acc_no"], $_POST["sender"], $reference, "success", "", $_GET["users"]);
        
        //log to activity log
		 $log_date = date("d/m/y");
		 $log_time = date("h:i A");
		 $log_activity = "Successfully Withdraw N".$am." from Wallet";
        ...
        header("location: ?r=home&users=".$_GET["users"]);
            ...
            $_SESSION["rrr"] = "<div class='d-flex flex-column align-items-center gap-3'><i class='bi bi-x-circle-fill fs-2 text-danger'></i> <span>The user does not have Sufficient Funds</span></div>";
            header("location: ?r=home&users=".$_GET["users"]."&selection=withdrawmoney");
        }
        ...
        $_SESSION["rrr"] = "Please Enter a figure toremove from the user's wallet";
        header("location: ?r=home&users=".$_GET["users"]."&selection=withdrawmoney");
    }
}
?>
<section style="position: fixed; left:0px;top:0px; width: 100%; height: 100%; background: rgba(0,0,0,0.5); display:flex; flex-direction: column; align-items: center; justify-content: center">
    <div class="rounded bg-white text-dark" style=" width:400px; overflow: hidden">
        <h1 class="text-center w-100 bg-primary text-white px-4 py-2">Withdraw Money</h1>
        <div class="py-3 px-3">
            <form method="post" action="">
                <div class="d-flex flex-column justify-content-center align-items-center">
                    <label>Amount (N)</label>
                    <input type="number" placeholder="0.00" name="amount" class="border border-1 border-primary rounded shadow lead p-2 fw-bold text-center" style="width:200px; height:initial">
                    <br>
                ...
                <button class="btn btn-primary rounded-pill w-100">Remove Money</button>
