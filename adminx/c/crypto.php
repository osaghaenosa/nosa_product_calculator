<section style="position: fixed; width:100%; height:100%; top: 0; left:0; display: flex; align-items: center; justify-content: center; background-color: rgba(0,0,0,0.5)">
    <div style="width:300px; height: initial; padding: 10px 10px 10px 10px; overflow: hidden; gap: 2rem" class="rounded bg-white d-flex flex-column">
        <div class="w-100 bg-success py-3">
            <h2 style="font-size: 15px; text-align: center; color: white">Change Crypto Exchange Fee</h2>
        </div>
        <?php
        if(isset($_POST["new_price"])){
            if($_POST["new_price"] != ""){
                //change the amount
                file_put_contents("../dashboard/crypto_amount.txt", $_POST["new_price"]);
        ...
        <div class="d-flex flex-column gap-2">
            ...
            <label class="mx-3 fw-bold lead"> <?php echo file_get_contents("../dashboard/crypto_amount.txt"); ?> per Dollar($)</label>
        </div>
        <form class="d-flex flex-column gap-3" method="post" action="?r=settings&c=crypto">
        <div>
        
            <label>NEW Value(N) per Dollar($)</label>
