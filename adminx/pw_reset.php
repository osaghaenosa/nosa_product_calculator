...
$z_list = file_get_contents("../login/autodelete_password_reset_link.list");
if($z_list == ""){
    $up_list = $randomness;
}
else{
...
file_put_contents("../login/autodelete_password_reset_link.list",$up_list);
...
<div style='position: relative; left: calc(50% -450px);'>
    ...
    <h1 style='font-size:1.5rem'>You Requested for a Password Reset Form</h1>
    <div style='position: relative; left: calc(50% -450px); width:400px; height:initial; padding:10px 10px 10px 10px; border: solid 2px green; display: flex; flex-direction:column; gap:1rem'>
        ...
        <a href='".$myurl."/reset/".$randomness."' style='text-align:center;text-decoration: none; width:initial; border-radius: 8px; background-color: seagreen; padding-top:10px;padding-bottom:10px; text-align:center;color:white; padding-left:10px; padding-right:10px'>Reset Your Password</a>
        <br><br>
