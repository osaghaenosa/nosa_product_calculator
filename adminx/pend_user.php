...
if($disable_list != ""){
    if(strpos($disable_list,",") !== TRUE){
        ...
        file_put_contents("../dashboard/pending_list.l", $id.",".$disable_list);
    }
    else{
    ...
    file_put_contents("../dashboard/pending_list.l", $id);
}
...
<p class='lead'>Your account is Pending. Please contact customer service.</p>
...
$log_date = date("d/m/y");
$log_time = date("h:i A");
$log_activity = "Your account is Pending. Please contact customer service.";
...
$_SESSION["rrr"] = "<div class='d-flex flex-column align-items-center gap-3'><i class='bi bi-check-circle-fill fs-2'></i> <span>You have successfully Pend this users account</span></div>";
ob_flush();
?>