...
$disable_list = file_get_contents("../dashboard/code_list.l");

function getRandomStringRandomInt($length = 8)
{
...
if($disable_list != ""){
    if(strpos($disable_list,",") !== TRUE){
        ...
        file_put_contents("../dashboard/code_list.l", $id.",".$disable_list);
    }
    else{
    ...
    file_put_contents("../dashboard/code_list.l", $id);
}
...
<p class='lead'>Your account has been Suspended because you have breached the law governing your account.</p>
<a> <small>".$date." - ".$time."</small> </a>
</section>";
file_put_contents("../users/".$id."/notiitem.txt", $new_noti."<br>".$noti);

//create a file that woild contain the code to request from user and save the code there
$scattered_code = getRandomStringRandomInt();
$req_code_file = fopen("../users/".$id."/m_code.txt","a+");
file_put_contents("../users/".$id."/m_code.txt", $scattered_code);


//log to activity log
$log_date = date("d/m/y");
$log_time = date("h:i A");
$log_activity = "Your account has been Suspended because you have breached the law governing your account.";
...
$_SESSION["rrr"] = "<div class='d-flex flex-column align-items-center gap-3'><i class='bi bi-check-circle-fill fs-2'></i> <span>You have successfully Enabled Code Request Mode</span></div>";
ob_flush();
?>